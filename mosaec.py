...
# structures failing during structure file parsing
ZeroVectorlist = []
...
for file in os.listdir(cwd):
    if all(
        ...
        for metal in noint_outer:
            ...
            if not any(
                [
                    math.isclose(float(noint_outer[metal][0]), float(i), abs_tol=0.5)
            ...
            if any(
                ...
                try:
                    prob = float(
                        ONProb[metal.atomic_symbol][round(noint_outer[metal][0])]
                    )
                except IndexError:
                    prob = 0
            ...
            if (noint_outer[metal][1] > 20) and (
                not mendeleev.element(metal.atomic_symbol).block == "f"
            ):
                high_count[metal] = "INSPECT"
            # for f-block, metal electron count above 32
            # is suspicious, flag for inspection
            if (noint_outer[metal][1] > 32) and (
                mendeleev.element(metal.atomic_symbol).block == "f"
            ):
                high_count[metal] = "INSPECT"
        ...
        for metal in noint_outer:
            ...
            if not any(
                ...
                if all(
                    [(ONEC_inner[metal][0] > valence), (ONEC_inout[metal][0] > valence)]
                ):
                    noint_flag[metal] = "BAD"
                # if non-integer after distribution and
                ...
                if not any(
                    [
                        math.isclose(float(ONEC_inner[metal][0]), float(i), abs_tol=0.5)
                        for i in KnownON[(metal.atomic_symbol)]
                    ]
                ):
                    impossible_valence[metal] = "BAD"
        ...
        for metal in ONEC_inner:
            ...
            if not any(
                [
                    math.isclose(float(ONEC_inout[metal][0]), float(i), abs_tol=0.5)
                    for i in KnownON[(metal.atomic_symbol)]
                ]
            ):
                pure_connect_error = True
            ...
            if any(
                ...
                try:
                    prob = float(
                        ONProb[metal.atomic_symbol][round(ONEC_inout[metal][0])]
                    )
                except IndexError:
                    prob = 0
                if prob < 0.01:
        ...
        # all flags set to GOOD if no error found for any metal in the structure
        # according to local distribution
        ...
        for metal in ONEC_inner:
            ...
            if not any(
                [
                    math.isclose(float(globaldis[metal][0]), float(i), abs_tol=0.5)
                    for i in KnownON[(metal.atomic_symbol)]
                ]
            ):
                global_error = True
            ...
            if any(
                ...
                try:
                    prob = float(
                        ONProb[metal.atomic_symbol][round(globaldis[metal][0])]
                    )
                except IndexError:
                    prob = 0
                if prob < 0.01:
        ...
        # all flags set to GOOD if no error found for any metal in the structure
        # according to global distribution
        ...
        # output os, ec, and error flags for each metal
        for metal in noint_outer:
            entrydata = pd.DataFrame(
                {
                    "CIF": [file],
